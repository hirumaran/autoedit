document.addEventListener('DOMContentLoaded', () => {
            ...
            if (e.target.files && e.target.files[0]) {
                ...
                try {
                    ...
                    if (data.success) {
                        ...
                        musicSelect.value = data.track_id;
                        selectedMusicFile = data.track_id;
    ...
    async function loadAvailableMusic() {
        try {
            ...
            if (musicSelect) {
                ...
                const tracks = data.tracks || data.music || [];
                tracks.forEach(m => {
                    const opt = document.createElement('option');
                    opt.value = m.id || m.filename || m.track_id;
                    opt.textContent = m.title || m.name || m.filename;
    ...
    async function pollStatus(videoId) {
        ...
        const interval = setInterval(async () => {
            try {
                ...
                const res = await fetch(`/api/download/status/${videoId}`);
                if (!res.ok) {
                    clearInterval(interval);
                ...
                if (data.status === 'analyzed') {
                    ...
                    // Job not yet in DB, keep polling until timeout
    ...
    async function populateReviewUI(videoId) {
        ...
        try {
            ...
            // Use already-fetched analysis data for duration and music
            window.currentVideoDuration = data.phase_one_metadata?.duration || data.analysis?.duration || 0;

            // Apply AI Suggested Music automatically
            if (data.ai_suggested_music) {
                const suggested = data.ai_suggested_music;
    ...
    if (confirmRenderBtn) {
        ...
        confirmRenderBtn.onclick = async () => {
            ...
            const burnSubs = document.getElementById('burn-subs-toggle').checked;
            console.log('burnSubs toggle value:', burnSubs, document.getElementById('burn-subs-toggle'));
