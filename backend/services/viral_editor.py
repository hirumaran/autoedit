...
class ViralEditor:
    ...
    def smart_sync_cuts(
        self, video_path: str, audio_track_id: str, suggested_cuts: List[Dict]
    ) -> List[Dict]:
        """
        ...
        if not audio_path:
            logger.warning(
                "Could not download audio for beat analysis, returning original cuts."
            )
        ...
        logger.info(
            f"🎵 Smart Sync: Found {len(beat_times)} beats. BPM: {analysis.get('bpm')}"
        )
    ...
    def apply_viral_edit(
        ...
        try:
            ...
            if not resolved_audio_track_id:
                raise ValueError(
                    "Missing audio track id. Provide `audio_track_id` or `music_id`."
                )
    ...
    def _render_with_moviepy(
        ...
        self._render_with_ffmpeg(
            video_in, audio_in, video_out, segments, bg_volume, is_preview
        )
    ...
    def _render_with_ffmpeg(
        ...
        if segments:
            ...
            volume_filter = (
                f"volume='if({ducks}, {bg_volume * 0.3}, {bg_volume})':eval=frame"
            )
